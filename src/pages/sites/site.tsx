  ...
  return (
    <>
      <Header
        ...
        subtitle={
          <>
            {data?.siteEmplacements?.result?.streetNumber}{" "}
            {data?.siteEmplacements?.result?.street} <br />
            {data?.siteEmplacements?.result?.postal}{" "}
            {data?.siteEmplacements?.result?.city}
          </>
        }
        buttons={[
          {
            ...
            bgColor: "indigo",
            textColor: "white",
            link: `/sites/${id}/update`,
            icon: <SendIcon />,
          },
        ]}
      />

      {!data?.siteEmplacements?.result?.completed && (
          ...
          <div className="p-6 bg-orange-50 border-l-4 border-orange-500 rounded-r-lg">
            <div className="flex items-center">
              <h3 className="text-orange-800 font-medium">
                La saisie de l'immeuble est incomplète
              </h3>
            </div>
          </div>
        </div>
      ...
      <div className="main-container">
          ...
          <div className="w-full xl:w-1/2   px-4 mb-4 md:mb-0">
            <Card>
              ...
              <table className="table-auto w-full">
                ...
                <tbody>
                  {data?.siteEmplacements?.result?.buildings.map(
                    (building, index) => (
                      <>
                        <tr
                        ...
                        >
                          <td className="padding-table flex ">
                            <div>
                              <p
                                ...
                                onClick={() =>
                                  navigate(
                                    `/sites/${id}/buildings/${building.id}`
                                  )
                                }
                              >
                                {building.name}
                              ...
                              <p className="text-gray-800">
                                {building.entrances?.length} entrée
                          ...
                          <td className="padding-table text-right">
                            <span
                              onClick={() =>
                                navigate(`/buildings/${building.id}/update`)
                              }
                        ...
                        {building?.entrances?.map((entrance) => (
                          ...
                          >
                            <td className="padding-table flex ">
                              <span className="w-5 h-1 block"></span>Entrée :{" "}
                              {entrance.name}
                            </td>

                            <td className="padding-table text-center ">
                              {
                                entrance.floors?.filter(
                                  (f) => f.type === FloorType.Stage
                                )?.length
                              }{" "}
                              etages -{" "}
                              {
                                entrance.floors?.filter(
                                  (f) => f.type === FloorType.Basement
                                )?.length
                              }{" "}
                              sous sol
                            </td>
                            ...
                            <td className="padding-table text-right ">
                              {entrance.floors.reduce(
                                (previousValue, currentValue) =>
                                  previousValue +
                                  currentValue.emplacements?.length,
                                0
                              )}{" "}
                              emplacements
                            </td>
        ...
        <div className="overflow-hidden">
          <div className="flex justify-between">
            <h4 className="font-semibold text-2xl mb-3">Contrats</h4>
          </div>
          <ContractsList
            emptyMessage="Aucun Contrat n'a encore été signé"
            where={{
              siteId: +id!,
        ...
        <div className="overflow-hidden">
          <div className="flex justify-between">
            <h4 className="font-semibold text-2xl mb-3">Propositions</h4>
          </div>
          <Button
        ...
        <div className="grid md:grid-cols-3 gap-x-5 gap-y-10 mt-5">
          <div className="overflow-hidden">
            <div className="flex justify-between">
              <h4 className="font-semibold text-2xl mb-3">En Attente</h4>
            </div>
            <ContractsList
              emptyMessage="Aucun Contrat n'est en attente"
              where={{
                siteId: +id!,
          ...
          <div className="overflow-hidden">
            <div className="flex justify-between">
              <h4 className="font-semibold text-2xl mb-3">Brouillon</h4>
            </div>
            <ContractsList
              emptyMessage="Aucune Proposition n'est en cours de validation"
              where={{
                siteId: +id!,
          ...
          <div className="overflow-hidden">
            <div className="flex justify-between">
              <h4 className="font-semibold text-2xl mb-3">Refusée</h4>
            </div>
            <ContractsList
              emptyMessage="Aucune Proposition n'a été refusée"
              where={{
                siteId: +id!,
