...
import { SITE, SITE_EMPLACEMENTS } from "../../../../queries/sites.queries";
import { CreateBuildingInput } from "../../../../__generated__/globalTypes";

interface ICreateBuildingModal {
  siteId: number;
  refetch: any;
}

export const CreateBuildingModal: React.FC<ICreateBuildingModal> = ({
  siteId,
  refetch,
}) => {
  const client = useApolloClient();
  const [isOpened, setIsOpened] = useState(false);
  const [mutate, { loading }] = useMutation(CREATE_BUILDING);
    ...
    try {
      ...
      if (data?.createBuilding.ok) {
        ...
        client.writeQuery({
          ...
          data: {
            siteEmplacements: {
              ...queryResult.siteEmplacements,
              result: {
                ...queryResult?.siteEmplacements?.result,
                buildings: [
                  {
                    __typename: "Building",
                  ...
                  ...queryResult?.siteEmplacements?.result?.buildings,
                ],
              },
            },
          },
  ...
  return (
    <>
      ...
      <Modal
        ...
        footer={null}
      >
        <BuildingForm form={form} submit={submit} loading={loading} />
      </Modal>
    </>
  );
};
