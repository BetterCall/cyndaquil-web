...
import { UPDATE_ENTRANCE } from "../../../queries/entrances.queries";
import {
  UpdateEntranceMutation,
  UpdateEntranceMutationVariables,
} from "../../../__generated__/UpdateEntranceMutation";

interface IEntranceSelector {
  id: number;
  name: string;
  isSelected: boolean;
  refetch: any;
...
export const EntranceSelector: React.FC<IEntranceSelector> = ({
  id,
  name,
  isSelected,
}) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      client.writeFragment({
        id: `Entrance:${id}`,
        fragment: gql`
          fragment EntranceUpdateed on Entrance {
            name
          }
        `,
        data: {
