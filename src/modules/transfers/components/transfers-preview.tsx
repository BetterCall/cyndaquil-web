...
import { TransferFiltersInput } from "../../../__generated__/globalTypes";
import { useLazyTransfers } from "../hooks";
import { EmptyList, Loading } from "../../../components";

interface TransferProps extends TransferFiltersInput {
  title?: string;
  subtitle?: string;
}

export const TransfersPreview: React.FC<TransferProps> = ({
  title,
  subtitle,
  date,
  ...where
  ...
  const [search, { data, loading }] = useLazyTransfers();
  useEffect(() => {
    if (filters === where) {
      return;
    }
      ...
      <div className="flex flex-wrap -m-2">
        <div className="w-full p-2">
          <div className="flex flex-wrap items-center pb-4 -m-2">
            {data?.transfers?.results?.map((transfer) => (
              <>
                <div className="w-full p-2">
                  <h3 className="mb-1 font-medium text-lg text-coolGray-900">
                    <span className="inline-block w-2 h-2 mr-2 rounded-full bg-green-500" />
                    {transfer?.id}
                  </h3>
