...
import { SearchEmplacementsInput } from "../components";
import { useLazyEmplacements } from "../hooks";

export const Emplacements: React.FC = () => {
  const navigate = useNavigate();
  const [limit, setLimit] = useState(10);

  const [search, { data, loading, fetchMore, error }] = useLazyEmplacements();
  ...
  return (
    <>
      <Header
        ...
        buttons={[
          {
            actionText: "Nouvelle emplacement",
            bgColor: "indigo",
            textColor: "white",
            link: `/emplacement/create`,
            icon: <SendIcon />,
          },
        ]}
      />
          ...
          <table className="table-auto w-full">
            <thead>
              <tr className="text-xs text-gray-500 text-left">
                <th className="padding-table  font-medium">Site</th>
              ...
              {data?.emplacements?.results?.map((emplacement, index) => (
                ...
                >
                  <td className="flex padding-table ">
                    <div>
                      <p
                        ...
                        onClick={() =>
                          navigate(`/emplacement/${emplacement.id}`)
                        }
                      >
                        {emplacement?.site?.name || "-"}
                      </p>
                  ...
                  <td className="padding-table text-right">
                    <span
                      onClick={() =>
                        navigate(`/emplacement/${emplacement.id}/update`)
                      }
          ...
          <div>
            <Button
              ...
              onClick={() => {
                fetchMore({
                  variables: {
                    offset: data?.emplacements?.results?.length,
                    limit,
                    where: parseSearchParams(searchParams),
                  },
                });
