...
import { useLazyEquipment } from "../../equipments/hooks";
...
export const ChangeEquipment: React.FC<IProps> = ({
  id,
  defaultValues,
  onCompleted,
  onError,
  ...
  const { form, submit, loading } = useUpdateEmplacement({
    ...
    onCompleted: () => {
      ...
      client.writeFragment({
        id: `Emplacement:${id}`,
        fragment: gql`
          fragment EmplacementEquipement on Emplacement {
            equipmentId
            equipment {
              id
              code
  ...
  const [fetchEquipment, { data }] = useLazyEquipment();
  const [oldEquipmentId, setEquipmentId] = useState<any>(null);
  ...
  return (
    <>
      <div
        ...
        className="cursor-pointer flex justify-center py-2 text-sm text-white bg-indigo-500 hover:bg-indigo-600 rounded transition duration-200"
      >
        <svg
          className="mr-2"
          width="18"
        ...
        <span>Changer l'equipement</span>
      </div>

      <Modal open={isModalOpen} onCancel={handleCancel} footer={null}>
        <CardHeader title="Changer l'equipement" />
              ...
              <div className="flex ">
                <div className="w-1/2 mr-1 ">
                  <div className="w-full mb-3">
                    <p className="label">Code de l'equipement</p>
              ...
              <div className="w-full pt-3">
                <FilesPreview
                  ...
                  where={{
                    ...
                    objectId: data?.equipment?.result?.id,
                  }}
                />
              </div>
            </div>
