...
export const ContactCategoriesInput: React.FC<ICustomerCategoriesInput> = ({
  form,
  disabled = false,
}) => {
  const { data } = useContactCategories();
  const value = form.watch("categoryId");
  return (
    <div className="w-full">
      <p className="label">Role du Contact</p>
      <div className="relative">
        ...
        >
          ...
          {data?.contactCategories?.results?.map((category) => {
            return (
              <option
                value={category.id}
