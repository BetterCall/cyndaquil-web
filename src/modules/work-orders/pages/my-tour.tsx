...
export const MyTour: React.FC = () => {
  const navigate = useNavigate();
  const { data } = useMe();
  const [date, _] = useState(moment());
  const { data: wData, loading } = useWorkOrders(
    { where: { userId: data?.me.id, date: date.format("YYYY-MM-DD") } },
    !data?.me.id
  );
  ...
  const renderList = () => {
    ...
    if (wData?.workOrders?.results?.length === 0) {
      ...
      return (
        <table className="table-auto w-full">
          ...
          <tbody>
            {wData?.workOrders?.results?.map((workOrder, index) => (
              <Row index={index} key={`workOrder-${workOrder.id}`}>
                ...
                <td className="padding-table text-right">
                  <span
                    onClick={() =>
                      navigate(`/work-order/${workOrder.id}/update`)
                    }
  ...
  return (
    <>
      <Header
        title={`Bonjour ${capitalizeFirstLetter(data?.me?.firstname ?? "")} `}
        subtitle={`Fiche de travail du ${date.format("dddd LL")} `}
      />
