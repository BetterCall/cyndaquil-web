...
import { useLazyWorkOrders } from "../../hooks/useWorkOrders";
...
export const WorkOrdersList: React.FC<WordOrderProps> = ({
  title,
  subtitle,
  date,
  ...where
  ...
  return (
    <Card>
      ...
      {!loading && data?.workOrders?.results?.length === 0 && (
        <div className="py-8 px-6 ">
          <div className="p-6 bg-gray-50  border-2 border-gray-500 rounded-lg">
            <div className="flex justify-center  w-full">
              <h3 className=" text-gray-800 font-medium ">
                Aucun Bon d'intervention n'est programmé
              </h3>
            </div>
          </div>
        </div>
      ...
      <div className="relative">
        ...
        <div className="relative">
          {data?.workOrders?.results?.map((workOrder) => (
            <div
              key={`order-order-${workOrder.id}`}
              className="flex items-center mb-8"
            >
              <span className="inline-block w-2 h-2 ml-px mr-4 rounded-full bg-green-500"></span>
              <div>
                <a
                  className="mb-1 text-sm font-medium flex align-middle justify-between  w-full"
                  href="#"
                >
                  <div className="flex items-center text-xs text-gray-500">
                    ...
                    <p>
                      {formatHour(workOrder.start)} -{" "}
                      {formatHour(workOrder.end)}
                    </p>
                  </div>
                </a>

                <div className="flex items-center text-xs ">
                  <p className="  text-sm font-medium">{workOrder.name}</p>
                </div>
                ...
                <div className="flex items-center text-xs mb-1">
                  <p>
                    {workOrder.postal} {workOrder.city}
                  </p>
                </div>
              </div>
            </div>
